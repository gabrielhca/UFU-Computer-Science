...
/*Considerando  o  exercício  anterior,  indique  duas  maneiras  diferentes  de 
calcular a soma dos elementos do vetor utilizando a variável p;
*p = a[1]+a[3];

Soma de todos os elementos usando p como iterador
for (int i = 0; i < 5; i++) {
    soma += *p;  // Soma o valor apontado por p
    p++;         // Avança o ponteiro para o próximo elemento
}

Soma de todos os elementos usando p como base e acessando com deslocamento
for (int i = 0; i < 5; i++) {
    soma += *(p + i);  // Acessa e soma cada elemento a partir de p
}
*/
